        ...
        <div>
          <div style={{ background:"#0d1117", border:"1px solid #1a2a3a", borderRadius:16, padding:"14px 16px", marginBottom:14 }}>
            ...
            {exercises.map((ex, idx) => (
              <div key={ex.uid||idx} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 0", borderBottom:"1px solid #1a2a3a" }}>
          ...
          <div style={{ background:"#0d1117", border:`1px solid ${wodLogged ? "#207050" : "#1a2a3a"}`, borderRadius:16, padding:"16px 18px", marginBottom:14 }}>
            ...
            {wodLogged ? (
              <div style={{ textAlign:"center", fontSize:22, fontWeight:900, color:"#50e090", padding:"10px 0" }}>✓ Loggat!</div>
            ) : (
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                <div style={{ display:"flex", gap:10 }}>
                  <div style={{ flex:2 }}>
                    <div style={{ fontSize:10, color:"#334455", letterSpacing:1, marginBottom:4 }}>{/amrap/i.test(day.focus) ? "RUNDOR" : "TID"}</div>
                    <input placeholder={/amrap/i.test(day.focus) ? "t.ex. 8+3" : "t.ex. 14:32"} value={wodResult} onChange={e => setWodResult(e.target.value)}
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <input placeholder="t.ex. 32" value={wodWeight} onChange={e => setWodWeight(e.target.value)} type="number"
                ...
                <button onClick={logWodResult} disabled={!wodResult}
                  style={{ padding:"13px", borderRadius:10, border:"none", cursor:wodResult?"pointer":"default", background:wodResult?`linear-gradient(135deg,${BLUE},${BLUE_DARK})`:"#1a2a3a", color:"#fff", fontWeight:800, fontSize:14 }}>Logga</button>
              </div>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ background:"#0a0e14", border:"1px solid #1a2a3a", borderRadius:14, padding:"14px 16px", marginTop:8 }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              ...
              <input type="number" value={restDuration} min={5} max={600} onChange={e=>setRestDuration(Number(e.target.value))}
                style={{ width:56, background:"#0a0e14", border:"1px solid #1a2a3a", borderRadius:8, padding:"7px 6px", color:"#d0e4f0", fontSize:13, outline:"none", textAlign:"center", fontWeight:700 }}/>
            </div>
          </div>
        </div>
      )}
